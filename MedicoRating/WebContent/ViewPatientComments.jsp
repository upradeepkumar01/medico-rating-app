...
String docter=(String)session.getAttribute("email");
...
String s=g.test123(user,docter);
int kk=s.indexOf('*');
if(s!="")
{
...
%>
<a class="fancybox fancybox.iframe" href="DocterCommentBox.jsp?a=<%=rs.getString("email")%>"><i class="icon-leaf icon-white"></i>Click here to Comment</a>
	<%}
else
{
	...
	<a href="DocterCommentBox.jsp?a=<%=rs.getString("email")%>" target="_blank">Click here to Comment</a>
...
<%}%>
  ...
  <tr>
    ...
    <td><a href="Unsubscribe.jsp?doctor=<%=rs.getString("email") %>"><b><center></center><b></b></a></td>
  </tr>

...
<div class="container_12 clearfix"> 
<div class="grid_12"> 
<div id="topnav" class="jqueryslidemenu"><ul>
...
<h1> Docters List by zipcode : <%=request.getParameter("country") %>  & speciality :  <%=request.getParameter("spl") %></h1>
</div>
...
String spl=request.getParameter("spl");
	//ps=cn.prepareStatement("select * from register where user=? and zipcode=? ");
  System.out.println("bfre");
	ps=cn.prepareStatement("select register.email,register.fname,register.photo,register.mobile,register.address1,register.city,register.zipcode,docter.specialities from register INNER JOIN docter ON register.email=docter.email  where  register.user=? and register.zipcode=? and docter.specialities=?");
	System.out.println("after");
	/*SELECT fabregister.id, fabregister.mname,fabdetails.duedate
	 FROM fabregister
	 INNER JOIN fabdetails
	 ON fabregister.id=fabdetails.id  where fabdetails.duedate<='2013-10-09';*/
...
<%}%>
  ...
  <tr>
    ...
    <td><a href="subscribe.jsp?doctor=<%=rs.getString("email") %>"><b><center>Click To Subscribe</center><b></b></a></td>
  </tr>

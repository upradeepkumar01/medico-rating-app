...
<h1> Docters List by speciality  : <%=request.getParameter("spl") %></h1>
</div>
...
String country=request.getParameter("spl");

	//ps=cn.prepareStatement("select * from register where user=? and zipcode=? ");
  System.out.println("bfre");
	ps=cn.prepareStatement("select register.email,register.fname,register.photo,register.mobile,register.address1,register.city,register.zipcode,docter.specialities from register INNER JOIN docter ON register.email=docter.email  where  register.user=?  and docter.specialities=?");
